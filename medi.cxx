/*                                SPECTRUM: C++
 * Copyright (C) Siemens AG 1992-2018 All Rights Reserved.Confidential
 * Message Display
 *
 * $NewRev
 * 190730 loid sp7-scada_v02.20_df_ems00112205_tlq_digitals_vie
 * 190102 vlandenb sp7-dots_v02.20_df_ems00111110_ib_msp
 *    fix ems00109998 also for DSOTS
 * 181217 Mayh sp7-scada_v02.20_df_ems00109998_medi_ots_case_restore_vie
 *    timing issue at OTS case restore response (SMUD project solution)
 * 171114 Mayh sp7-scada_v02.20_df_ems00095613_medi_background_update_vie
 * 170908 ekaul EMS00090929 make HIS subtypes configurable
 * 170421 ekaul ems00087431 replay in OTS
 * 170410 vlandenb sp7-dots_v02.20_fu_ems00087029_eots-dots_msp
 *                 allow EOTS and DOTS processes on same machine
 * 170503 Mayh sp7-scada_v02.20_df_ems00087812_coeldeta_320infos_vie 
 * 170105 Mayh sp7-scada_v02.10_df_ems00083488_medi_detacheck_vie
 *   ATTENTION: internal decision table storage:
 *              DeTaPointer array index shifted (+1)
 ...
 *
 * $NewRev$
 * 170505 ekaul sp7-any_v02.20_df_ems00088005_ots_replay_msp Pick up EMS00087431
 * $NewRevEnd$
 */
...
struct tDeTaEntry {
  ...
  inline bool notEl() {           //not DtEl data
    if (!ElDeTa) return true;
    if (UU.El == NULL) return true;
    return false;
  }
  inline bool notTy() {           //not DtTy data
    if (ElDeTa) return true;
...
tDeTaEntry    DeTaPointer[cDTKeyMax+1];
...
for (i = 0; i<=cDTKeyMax; ++i) {
  free(DeTaPointer[i].UU.El);
  DeTaPointer[i].UU.El = 0;
  DeTaPointer[i].ElDeTa = false;
  }

fCoDeTaEl.Rp->DtNr = 0;
DbPos(&fCoDeTaEl,NxKey);
while (DbRead(&fCoDeTaEl,NextRc)) {
  DeTaPointer[_AC(fCoDeTaEl.Rp->DtNr,cDTKeyMax)].UU.El =
    (CoDeTaEl*)malloc_chk(fCoDeTaEl.RcS);
  DeTaPointer[fCoDeTaEl.Rp->DtNr].ElDeTa = true;
  ByIdBy(DeTaPointer[fCoDeTaEl.Rp->DtNr].UU.El,&vCoDeTaElL,fCoDeTaEl.RcS);
  ++NCoDeTaEl;
  }
fDeTa.Rp->DtNr = 0;
DbPos(&fDeTa,NxKey);
while (DbRead(&fDeTa,NextRc)) {
  if (!DeTaPointer[_AC(fDeTa.Rp->DtNr,cDTKeyMax)].ElDeTa) {
    DeTaPointer[fDeTa.Rp->DtNr].UU.TY =
      (DeTa*)malloc_chk(fDeTa.RcS);
    ByIdBy(DeTaPointer[fDeTa.Rp->DtNr].UU.TY,&vDeTaL,fDeTa.RcS);
    ++NDeTa;
  } else {
    errfmt(0,3201,"FillDeTa","pFillCoDeTa: duplicate data base entry El/Ty: %d - DeTa not used!"
                 ,fDeTa.Rp->DtNr);
    free(DeTaPointer[fDeTa.Rp->DtNr].UU.El);
    DeTaPointer[fDeTa.Rp->DtNr].UU.El = NULL;
    DeTaPointer[fDeTa.Rp->DtNr].ElDeTa = false;
  }
}
...
for (int il=0; il <= cDTKeyMax; ++il) {
  ...
  if (lDtErr) {
    errfmt(0,3202,"FillDeTa","pFillCoDeTa: wrong Elem/Info: %d (%s) - DeTa not used!"
                 ,il,(DeTaPointer[il].ElDeTa ? "El" : "Ty"));
...
for (i = 0; i<=cDTKeyMax; ++i) {
  if (DeTaPointer[i].ElDeTa) {
    free(DeTaPointer[i].UU.El);
    DeTaPointer[i].UU.El = 0;
    DeTaPointer[i].ElDeTa = false;
  } else {
    free(DeTaPointer[i].UU.TY);
    DeTaPointer[i].UU.TY = 0;
    }
  }
}  //pClearCoDeTa
...
bool          GetInfoSetEl(short DtNr, tinfoSet* infoSet)
{
  ...
  if (DeTaPointer[DtNr].notEl()) {
    DatErr(0,2315,"DT inval",0,0,DtNr,"III","CoDeTaEl",TA,0);
    errfmt(0,2315,"DT inval","GetInfoSetEl: no CoElDeTa: DtNr: %d",DtNr);
    return false;
  }

  for (int j = 0; j < DeTaPointer[DtNr].UU.El->NrInfo; ++j)
    AddSet(*infoSet,DeTaPointer[DtNr].UU.El->Info[j],tinfoSet);
...
if (!cUseMeDispOrig) {
  errfmt(0,10,"Init    ","pMeSoftInit: Don't write MeDisp, DelAllRc MeDisp");
...
for (i=0; i<=neDim; i++) {
  ...
  if (pNede == NULL) continue;

  // open line stat infos
  if ((i == neTopElem) && (pNede->NezuInf > 0)) {
  ...
  if ((!IsMultiPhaseYes) || (!IsNoElMP(i))) continue;
...
//  if (!DeTaPointer[_AC(ideTaNA,cDTKeyMax)].ElDeTa) return;
//
//  CoDeTaEl* pCoDt = DeTaPointer[ideTaNA].UU.El;
//
...
memset(Re.ist,0,sizeof(Re));
memcpy(Re.ist,ist,iln);
ContinueRu = true;

pWTHEl = DeTaPointer[DtNr].UU.El;
...
Ok = true;
AcEx = 0;
if ((DtPara->DtNr < 0) || (DtPara->DtNr>cDTKeyMax)) {
  err(0,2601,"DTNR    ",DtPara->DtNr,cDTKeyMax,0.0,"III");
  *Flag = 50;
  return;
  }
// 2
if (DeTaPointer[DtPara->DtNr].notTy()) {
...
pWTHTY = DeTaPointer[DtPara->DtNr].UU.TY;
if (Ok) {  // 3
...
FORLIM = DeTaPointer[_AC(DtNr,cDTKeyMax)].UU.TY->NrCond;
for (int j = 0; j < FORLIM; ++j) {
  pWTHj = &DeTaPointer[DtNr].UU.TY->uD.cl[j];
  //with CL
  if (nbTA==0) {
    ...
    k = 0;
    while (k<nbTA && !found) {
      if (pWTHj->Elem==bTAcrmd[_AC(k,Condmax-1)].TA.Elem &&
          pWTHj->B3==bTAcrmd[k].TA.B3 && pWTHj->B2==bTAcrmd[k].TA.B2 &&
          pWTHj->B1==bTAcrmd[k].TA.B1) {
        found = true;
        infoS = pWTHj->Info;
        AddSet(bTAcrmd[k].bInfoSet,infoS,tiinfoSet);
        }
      ++k;
      }
    if (!found) {
...
if (DtNr!=0) {
  ...
  FORLIM = DeTaPointer[DtNr].UU.TY->NrCond;
  for (j = 0; j < FORLIM; ++j) {
    if (InSet(pmeVar->proMode,(int)deTaTyp,sProMode)) {  //typisiert
      DeTaPointer[DtNr].UU.TY->uD.cl[_AC(j,Condmax-1)].B1 =
        pmeVar->taMeVar.uCmb.sTa4.B1;
      DeTaPointer[DtNr].UU.TY->uD.cl[j].B2 = pmeVar->taMeVar.uCmb.sTa4.B2;
      DeTaPointer[DtNr].UU.TY->uD.cl[j].B3 = pmeVar->taMeVar.uCmb.sTa4.B3;
      }
    }
...
if (DtNr!=0) {
  ...
  for (j = 0; j < FORLIM; ++j) {
    if (InSet(meVar->proMode,(int)deTaTyp,sProMode)) {  //typisiert
      ...
      DeTaPointer[DtNr].UU.TY->uD.cl[j].B2 = meVar->taMeVar.uCmb.sTa4.B2;
      DeTaPointer[DtNr].UU.TY->uD.cl[j].B3 = meVar->taMeVar.uCmb.sTa4.B3;
      }
    }
...
if (pmeVar->deTaNA==0) {
  ...
  CleSet(bMeDisp.varList[_AC(bMeDisp.nVar-1,cMaxMeDisp-1)].V.infoSet,tinfoSet);
  FORLIM = DeTaPointer[DtNr].UU.El->NrInfo;
  for (j = 0; j < FORLIM; ++j) {
    infoS = DeTaPointer[DtNr].UU.El->Info[_AC(j,Infomax-1)];
...
if (receive.DMSDId.dId==0)
  ...
  if (!McTransmit(&sbflag,McOwnLevel(),s_HIS_TOPR,true,
    ...
    if (NimVarMp.NedePoi[_AC(
      ...
      if (!InSet(pmeDisp->varList[_AC(varInd-1,cMaxMeDisp-1)].infoSet,
        ...
        if (!insBTA(dId,current_RelCopKey,
          ...
          RawInf& rawInf = iRawInf.rRawInf[iRawInf.numberRaw];
          //Build header of raw information.
          rawInf.Header1 = obMEPR;
          rawInf.Header2 = RawMes;
          rawInf.RelCopKey = current_RelCopKey;
          rawInf.Sec = gActSec;
          rawInf.mSec = gActMSec;
          //2.timestamp
          rawInf.SecSys = gActSec;
          rawInf.mSecSys = gActMSec;

          //Build "obMEPR" part of raw information.
          rawInf.uRawI.sMepr.TyDeNu.InfoType = NimVarTy.pInTyDe->InfoType;
          rawInf.uRawI.sMepr.NimSatz = NimVarMp.NimSatz;
          rawInf.uRawI.sMepr.TA = *NimVarMp.pTa;
          rawInf.uRawI.sMepr.TA.Info = pWTHTA->Info;
          rawInf.uRawI.sMepr.NoElType = NimVarMp.NoElTy;
          rawInf.uRawI.sMepr.meClass = NimVarTy.pInTyDe->MeldeKlasse;
          rawInf.uRawI.sMepr.Priority= NimVarTy.pInTyDe->Priority;
          rawInf.uRawI.sMepr.Source= Id_SrcScada;

          //Build "RawUni" part of raw information.
          rawInf.uRawI.sMepr.uRVal.sRUni.NewValue = 1;
          memcpy(rawInf.uRawI.sMepr.uRVal.sRUni.InfStr,
                 NimVarMp.pInfStr,NimVarMp.StrLeng[NimVarMp.NoElTy]);
          rawInf.uRawI.sMepr.uRVal.sRUni.NStatChgR = 1;
          AddSet(rawInf.uRawI.sMepr.uRVal.sRUni.ChanSubI, SubAckn, tSetSubInf);
          ++iRawInf.numberRaw;

          if (iRawInf.numberRaw==cRawInf) {
            pTransmVasd(iRawInf);
            iRawInf.numberRaw = 0;
            }
          }
...
V.LNK_pVarDisplay = LNK_pVarDisplay;
FORLIM = DeTaPointer[_AC(DtNr,cDTKeyMax)].UU.TY->NrCond;
for (int jC = 0; jC < FORLIM; ++jC) {
  pWTHjC = &DeTaPointer[DtNr].UU.TY->uD.cl[jC];
  //with CL
  found = false;
  V.k = 1;
...
if (pWTHiWVipShort->vipAct==vipActLFig || pWTHiWVipShort->vipAct==vipActGFig)
  ...
  lElDtOk = true;
  if (pWTHpmeVar->deTaNA != 0) {
    lDetaIdx = _AC(pWTHpmeVar->deTaNA,cDTKeyMax);
  ...
  if ((lElDtOk) && (pWTHpmeVar->deTaNF != 0)) {
    lDetaIdx = _AC(pWTHpmeVar->deTaNF,cDTKeyMax);
    lElDtOk = (((InSet(pWTHpmeVar->proMode,(int)deTaEl,sProMode))  &&
                (DeTaPointer[lDetaIdx].ElDeTa))
            || ((!InSet(pWTHpmeVar->proMode,(int)deTaEl,sProMode)) &&
                (!DeTaPointer[lDetaIdx].ElDeTa)));
...
if (rawInfVasd.numberRaw>0)
  pTransmVasd(rawInfVasd);
...
void          pTransmVasd(
  tRawInf&      iRawInf)
{
  /* Only one RawInf is send to vasd */
  size_t sbsize = (sizeof(tRawInf) - (cRawInf * sizeof(RawInf)) +
                   (sizeof(RawInf) * iRawInf.numberRaw));

  iRawInf.Header1 = obMEPR;
  iRawInf.Header2 = RawBloc;
  iRawInf.RelCopKey = iRawInf.rRawInf[0].RelCopKey;

  if (!SbTransm(&sbF,OwnComput,saVArec2,obVASD,sbsize,&iRawInf)) {
    err(0,3211,"VASD",rs("SBTR"),sbF,saVArec2,"SII");
  }
}
...
tRawInf       rawInfVasd{};
RawInf&       rawInf = rawInfVasd.rRawInf[0];
...
if (!ackOk) {
  ...
  //Build header of raw information.
  rawInfVasd.numberRaw = 1;
  rawInf.Header1 = obMEPR;
  rawInf.Header2 = RawMes;
  rawInf.RelCopKey = saveSingAck.RelCopKey;
  rawInf.Comput = saveAckpara.Comput;
  rawInf.Sec = gActSec;
  rawInf.mSec = gActMSec;
  //2.timestamp
  rawInf.SecSys = gActSec;
  ...
  //Build "obMEPR" part of raw information.
  rawInf.uRawI.sMepr.TyDeNu.InfoType = saveInfoType;
  rawInf.uRawI.sMepr.NimSatz = saveNimSatz;
  rawInf.uRawI.sMepr.TA = saveTA;
  rawInf.uRawI.sMepr.TA.Info = varInfo;
  rawInf.uRawI.sMepr.NoElType = saveNoElTy;
  rawInf.uRawI.sMepr.meClass = NimVarTy.pInTyDe->MeldeKlasse;
  rawInf.uRawI.sMepr.Priority= NimVarTy.pInTyDe->Priority;
  rawInf.uRawI.sMepr.Source= Id_SrcScada;

  //Build "RawUni" part of raw information.
  rawInf.uRawI.sMepr.uRVal.sRUni.NewValue = 0;
  memcpy(rawInf.uRawI.sMepr.uRVal.sRUni.InfStr,
         pInfoStr,NimVarMp.StrLeng[saveNoElTy]);
  rawInf.uRawI.sMepr.uRVal.sRUni.Opm = saveOpId;
  rawInf.uRawI.sMepr.uRVal.sRUni.NStatChgR = 1;
  AddSet(rawInf.uRawI.sMepr.uRVal.sRUni.ChanSubI, SubAckn, tSetSubInf);
  pTransmVasd(rawInfVasd);
...
if (AcknPermit(saveVipDispAck.AckPara.Comput,saveVipDispAck.chan)) {
  ...
  if (LNK_pFulSegAck->ackOk) {
    ...
    rawInf.uRawI.sMepr.NimSatz = NimVarMp.NimSatz;
    rawInf.uRawI.sMepr.TA = *NimVarMp.pTa;
    rawInf.uRawI.sMepr.TA.Info = LNK_pFulSegAck->varInfo;
    rawInf.uRawI.sMepr.NoElType = NimVarMp.NoElTy;
    rawInf.uRawI.sMepr.meClass = NimVarTy.pInTyDe->MeldeKlasse;
    rawInf.uRawI.sMepr.Priority= NimVarTy.pInTyDe->Priority;
    rawInf.uRawI.sMepr.Source= Id_SrcScada;
    ...
    rawInf.uRawI.sMepr.uRVal.sRUni.NewValue = 0;
    memcpy(rawInf.uRawI.sMepr.uRVal.sRUni.InfStr,
           pInfoStr,NimVarMp.StrLeng[NimVarMp.NoElTy]);
    rawInf.uRawI.sMepr.uRVal.sRUni.NStatChgR = 1;
    AddSet(rawInf.uRawI.sMepr.uRVal.sRUni.ChanSubI, SubAckn, tSetSubInf);
    ++iRawInf.numberRaw;
...
while (iVipDispAck<=saveVipDispAck.dataL &&
  ...
  while (V.iR<=pMeOut->nMeOut && xyNoMatch) {
    ...
    if (pttVipShort->Mark!=0 &&
        ...
        InSet(pmeVar->proMode,(int)ackPerm,sProMode)) {
      xyNoMatch = false;
      ackVar(&V,rawInfVasd);      //Acknowledge variable
      }
    ++V.iR;
    }
  ++iVipDispAck;
...
for (iR = 1; iR<=FORLIM; ++iR) {
  ...
  if (ackOk) {
    ...
    RawInf& rawInf = rawInfVasd.rRawInf[rawInfVasd.numberRaw];
    //Build header of raw information.
    rawInf.Header1 = obMEPR;
    rawInf.Header2 = RawMes;
    rawInf.RelCopKey = saveMeCoAck.RelCopKey;
    rawInf.Sec = gActSec;
    rawInf.mSec = gActMSec;
    //2.timestamp
    rawInf.SecSys = gActSec;
    ...
    rawInf.uRawI.sMepr.uRVal.sRUni.NStatChgR = 1;
    AddSet(rawInf.uRawI.sMepr.uRVal.sRUni.ChanSubI, SubAckn, tSetSubInf);
    ++rawInfVasd.numberRaw;

    if (rawInfVasd.numberRaw==cRawInf) {
      pTransmVasd(rawInfVasd);
      rawInfVasd.numberRaw = 0;
      }
    }  //ackOk
  }
...
for (Ix = 1; Ix<=FORLIM; ++Ix) {
  ...
  if (ackOk) {
    ...
    rawInf.Header1 = obMEPR;
    rawInf.Header2 = RawMes;
    rawInf.RelCopKey = saveApiAck.SboHead.RelCopKey;
    rawInf.Sec = gActSec;
    rawInf.mSec = gActMSec;
    //2.timestamp
    rawInf.SecSys = gActSec;
    ...
    rawInf.uRawI.sMepr.NimSatz = NimVarMp.NimSatz;
    rawInf.uRawI.sMepr.TA = *NimVarMp.pTa;
    rawInf.uRawI.sMepr.TA.Info = pWTHIx->TechId.Info;
    rawInf.uRawI.sMepr.NoElType = NimVarMp.NoElTy;
    rawInf.uRawI.sMepr.meClass = NimVarTy.pInTyDe->MeldeKlasse;
    rawInf.uRawI.sMepr.Priority= NimVarTy.pInTyDe->Priority;
    rawInf.uRawI.sMepr.Source= Id_SrcScada;
  ...
  while (PtB.nextBDispI!=0) {
    ...
    while (PtB.nextBTA!=0) {
      ...
      if (PtB.nextBTA->acknFlash==true &&
        ...
        bFlash = PtB.bSelect;
        totalAckTA(rawInfVasd);   //Update acknowledged TA's
        }
  ...
  case obDIMA:
    switch (receive.outDisp.Header2) {
      case joutDisp:
        ...
        if (receive.outDisp.dId!=0) {
          if (inpComput < cFirstComput || inpComput>=cMaxComput) {
            okInputMe_RTN = false;
...
if (testChan) {
  if (inpComput < cFirstComput || inpComput>=cMaxComput || inpChan<=0 ||
      inpChan>=cMaxChan || mmiDir[_AC(inpComput-1,cMaxComput-1)]==NULL ||
      mmiDir[inpComput-1]->VChan[_AC(inpChan-1,cMaxChan-1)]==NULL) {
    okInputMe_RTN = false;
    errfmt(0,1201,"MAIN    ","okInputMe-testChan: incorrect comput/chan: H1/H2 :%d/%d, inpComput/inpChan: %d/%d"
                            ,receive.U1.Header1,receive.U1.Header2,inpComput,inpChan);
      ...
      case s_DIM_ME:
        switch (receive.SboH.Request) {
          ...
          if (MyOpMode == opmTraining) {
            ...
            ObjectIdS OtsAckObject = receive.OtsSbOrderAck.AckObject;
            SbAddressS OtsAckAddress = receive.OtsSbOrderAck.AckAddress;
            ByteInt OtsSubRequest = receive.OtsSbOrderAck.SubRequest;
            ...
            if (OtsSubRequest == 0) {
              ...
              if (!SbTransm(&sbF,ots_computer,OtsAckAddress,OtsAckObject,
                            sizeof(tSboHead),&OTS_AckSb))
                err(0,3211,"mainMedi",rs("SBTR"),ri(sbF),
                    ri(OtsAckAddress),"sii");
              else
                cp(0,2003,"TRAINING",rs(otstype),rs("LOAD"),rs("DONE"),"sss");
